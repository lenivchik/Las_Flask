...
def open_file(file_ref, **encoding_kwargs):
    ...
    See :func:`lascheck.reader.open_with_codecs` for keyword arguments that can be
    used here.
...
def open_with_codecs(
    ...
    This function is called by :func:`lascheck.reader.open_file`.
...
def parse_header_section(
    ...
    Arguments:
        sectdict (dict): object returned from
            :func:`lascheck.reader.read_file_contents`
    ...
    Keyword Arguments:
        ignore_header_errors (bool): if True, issue HeaderItem parse errors
            ...
            :exc:`lascheck.exceptions.LASHeaderError` exception.
    ...
    Returns:
        :class:`lascheck.las_items.SectionItems`
    ...
    header_errors = []  # <-- COLLECT ERRORS INSTEAD OF RAISING
    ...
    for i in range(len(sectdict["lines"])):
        ...
        try:
            ...
            message = 'строка - {} (секция {}): "{}"'.format(j, title, line)
            header_errors.append(message)  # <-- COLLECT ERROR
            if ignore_header_errors:
                logger.warning(message)
                continue  # <-- CONTINUE PROCESSING
            else:
                logger.error(message)
                continue  # <-- CONTINUE PROCESSING EVEN WITH ERRORS
        else:
    ...
    if hasattr(section, '__dict__'):
        section.header_errors = header_errors
...
class SectionParser(object):
    ...
    def __call__(self, **keys):
        ...
        Refer to :meth:`lascheck.reader.SectionParser.metadata`,
        :meth:`lascheck.reader.SectionParser.params`, and
        :meth:`lascheck.reader.SectionParser.curves` for the methods actually
        used by this routine.

        Keyword arguments should be the key:value pairs returned by
        :func:`lascheck.reader.read_header_line`.
...
def read_line(*args, **kwargs):
    ...
    See :func:`lascheck.reader.read_header_line`.
